 ...
 * @returns {boolean}
 */
var isEmpty = function (item, withZero) {
    var MY_EMPTY = EMPTY.slice();
 ...
 */
function addDebtorToGrid (entities, eventArgs) {
    var grid = eventArgs.commons.api.grid;
    var viewState = eventArgs.commons.api.viewState;
    var entity = 'DebtorEntity';
    var debtor;

    if (!isEmpty(entities.OperationCRUDFormEntity.fullName) &&
        isEmpty(entities.OperationCRUDFormEntity.idNumber) &&
        ...
        var mainDebtor = false,
            alreadyRegisterDebtor = false,
            emptyDebtor;

        entities[entity].data().forEach(function (_debtor) {
        ...
        emptyDebtor = [debtor.fullName, debtor.idNumber, debtor.code].some(isEmpty);
 ...
 */
function  suprColAmortization (entities, eventArgs,gridId) {
    var grid = eventArgs.commons.api.grid,
    viewState = eventArgs.commons.api.viewState;
