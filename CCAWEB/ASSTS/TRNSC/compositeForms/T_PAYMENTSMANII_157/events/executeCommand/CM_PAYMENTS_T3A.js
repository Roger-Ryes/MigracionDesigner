    ...
    task.executeCommand.CM_PAYMENTS_T3A = function(entities, executeCommandEventArgs) {
        ...
        if (executeCommandEventArgs.commons.api.viewState.getDataItem("VA_TEXTINPUTBOXCFY_310141").pCobis == 4) { // ['Nota de DEbito a Cuenta de Aho']
           ...
           if (entities.Payment.finePrepayment > 0){
            ...
            quotationValue = getQuotation(entities.QuotationCurrency, entities.Payment.operationCurrencyType)
                entities.Payment.quotationValue = quotationValue

                quotationValue = 0.0;
            quotationValue = getQuotation(entities.QuotationCurrency, entities.Payment.currencyType)
                entities.Payment.payQuotationValue = quotationValue

                quotationValue = 0.0;
            quotationValue = getQuotation(entities.QuotationCurrency, entities.LeftOverPayment.currencyType)
            ...
            entities.PaymentGroup.operacion = ioperacionGrupal;
        }
        ...
        if (entities.Payment.value < entities.Payment.amountPrepayment) {
            ...
            return executeCommandEventArgs.commons.messageHandler.showMessagesConfirm("ASSTS.MSG_ASSTS_MONTOESEC_40993").then(
                 function (response) {
