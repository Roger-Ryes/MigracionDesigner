      ...
      switch (rate.signMin) {
        ...
        case "-":
          if (rate.value - rate.valueMin < 0) {
            eventArgs.commons.messageHandler.showMessagesInformation("ASSTS.MSG_ASSTS_MNIMODERI_76453", false, null, this.ASSTS.timer);
            rate.valueMin = 0;
          } else {
            rate.lockedMin = rate.value - rate.valueMin;
          }
      ...
      switch (rate.signMax) {
        ...
        case "-":
          if (rate.value - rate.valueMax < 0) {
            eventArgs.commons.messageHandler.showMessagesInformation("ASSTS.MSG_ASSTS_MXIMODERL_75830", false, null, this.ASSTS.timer);
            rate.valueMax = 0;
          } else {
            rate.lockedMax = rate.value - rate.valueMax;
          }
  ...
  task_executeCommand_CM_TRATESMO_TU_ = (
    ...
    if (entities.Rates.clase == 'F') {
      if (entities.Rates.referenceValue == '') {
        executeCommandEventArgs.commons.messageHandler.showMessagesInformation("ASSTS.MSG_ASSTS_SEDEBEIEI_21995", false, null, this.ASSTS.timer);
        executeCommandEventArgs.commons.execServer = false;
        return;
      }
      if (entities.Rates.typePoints == '') {
        executeCommandEventArgs.commons.messageHandler.showMessagesInformation("ASSTS.MSG_ASSTS_DEBESELNT_56184", false, null, this.ASSTS.timer);
        executeCommandEventArgs.commons.execServer = false;
        return;
      }

      if (entities.Rates.signDefault == '' || entities.Rates.signMin == '' || entities.Rates.signMax == '') {
        executeCommandEventArgs.commons.messageHandler.showMessagesInformation("ASSTS.MSG_ASSTS_SEDEBENST_55237", false, null, this.ASSTS.timer);
        executeCommandEventArgs.commons.execServer = false;
        return;
      }
    }
    ...
    if (entities.Rates.lockedMin! > entities.Rates.lockedMax!) {
      executeCommandEventArgs.commons.messageHandler.showMessagesInformation("ASSTS.MSG_ASSTS_VALORMNIA_88442", false, null, this.ASSTS.timer);
      executeCommandEventArgs.commons.execServer = false;
      return;
    }
    if (entities.Rates.lockedDefault! < entities.Rates.lockedMin! || entities.Rates.lockedDefault! > entities.Rates.lockedMax!) {
      executeCommandEventArgs.commons.messageHandler.showMessagesInformation("ASSTS.MSG_ASSTS_VALORESDA_30202", false, null, this.ASSTS.timer);
      executeCommandEventArgs.commons.execServer = false;
      return;
    }
